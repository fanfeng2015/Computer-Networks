...
/*
 * HTTP server using thread pool with shared queue busy wait.
 */
public class HTTPServerSharedQueueBusyWait {
    ...
    public static void main(String[] args) throws Exception { // assume that args follows the required format
        ...
        for (int i = 0; i < config.getThreadPoolSize(); i++) {
            threads[i] = new ServiceThreadSharedQueueBusyWait(config, fileCache, queue);
            threads[i].start();
        }
