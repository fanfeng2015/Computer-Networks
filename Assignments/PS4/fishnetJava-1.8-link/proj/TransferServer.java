...
public class TransferServer extends FishThread {
     ...
     */
    public TransferServer(Manager manager, Node node, TCPSock serverSock, long serverInterval, long workerInterval, int sz) {
        super(manager, node);
    ...
    public TransferServer(Manager manager, Node node, TCPSock serverSock) {
        this(manager, node, serverSock, DEFAULT_SERVER_INTERVAL, DEFAULT_WORKER_INTERVAL, DEFAULT_BUFFER_SZ);
    }

    public void execute() {
        if (!serverSock.isClosed()) { // check state
            ...
            TransferWorker worker = new TransferWorker(manager, node, connSock, workerInterval, sz);
            worker.start();
        } else {
    ...
    private class TransferWorker extends FishThread {
        ...
        public TransferWorker(Manager manager, Node node, TCPSock sock, long interval, int sz) {
            super(manager, node);
            this.sock = sock;
