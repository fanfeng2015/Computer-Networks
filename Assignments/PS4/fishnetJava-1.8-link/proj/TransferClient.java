...
public class TransferClient extends FishThread {
    ...
    public TransferClient(Manager manager, Node node, TCPSock sock, int amount, long interval, int sz) {
        super(manager, node);
        this.sock = sock;
    ...
    public TransferClient(Manager manager, Node node, TCPSock sock, int amount) {
        this(manager, node, sock, amount, DEFAULT_CLIENT_INTERVAL, DEFAULT_BUFFER_SZ);
    }
    ...
    public void execute() {
        if (sock.isConnectionPending()) {
            ...
            node.logOutput("time elapsed = " + (finishTime - startTime) + " msec");
