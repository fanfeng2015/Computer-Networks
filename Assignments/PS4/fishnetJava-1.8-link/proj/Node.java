...
/**
 * <pre>
 * Node -- Class defining the data structures and code for the protocol stack for a node
 * participating in the fishnet.

 * This code is under student control; we provide a baseline "hello world" example just to
 * show how it interfaces with the FishNet classes.
 *
 * NOTE: per-node global variables are prohibited.
 *
 * In simulation mode, we execute many nodes in parallel, and thus they would share any 
 * globals. Please put per-node state in class Node instead.
 *
 * In other words, all the code defined here should be generic -- it shouldn't matter to
 * this code whether it is executing in a simulation or emulation.
 *
 * This code must be written as a state machine -- each upcall must do its work and return
...
public class Node {
    ...
    // Fishnet reliable data transfer
    /**
     * Send a transport segment to the specified node (network layer service entry point
    ...
    private void receivePacket(int from, Packet packet) {
    	switch(packet.getProtocol()) {
        	...
        	case Protocol.PING_REPLY_PKT:
        	    ...
        	    break;
            case Protocol.TRANSPORT_PKT: // support of transport packet protocol
    ...
    // Receive a Fishnet packet, unpack to get segment and hand to manager.
    ...
    private boolean matchTransferCommand(String command) { // sender
        ...
        try {
            ...
            sock.bind(localPort);
            sock.connect(destAddr, port); // sender socket (localAddr, localPort, remoteAddr, remotePort)
    ...
    private boolean matchServerCommand(String command) {
        ...
        try {
            ...
            TCPSock sock = this.tcpMan.socket();
            sock.bind(port); // receiver socket (localAddr, localPort, *, *)
            sock.listen(backlog);
    ...
    private boolean matchPingCommand(String command) {
        ...
        try {
            ...
            Packet packet = new Packet(destAddr, this.addr, Packet.MAX_TTL, Protocol.PING_PKT, 0, Utility.stringToByteArray(message));
    ...
    private void addTimer(long deltaT, String methodName) { // timeout
        try {
...
// Node respresents the network layer, socekt should generate the segment and send it
// the transport's content is 
// from network layer, payload is segement
// from trans
// understand pack/unpack
